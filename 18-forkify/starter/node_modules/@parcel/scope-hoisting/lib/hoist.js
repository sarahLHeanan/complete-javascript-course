...
const REEXPORT_TEMPLATE = _template().default.statement('$parcel$reexport(EXPORTS, NAME, function(){return LOCAL;});');
...
const VISITOR = {
  ...
  MemberExpression(path, asset) {
    ...
    if (t().matchesPattern(path.node, 'module.bundle.root') || t().matchesPattern(path.node, 'module.bundle')) {
  ...
  ExportNamedDeclaration(path, asset) {
    ...
    if (source) {
      for (let specifier of (0, _nullthrows().default)(specifiers)) {
        ...
        path.insertAfter(REEXPORT_TEMPLATE({
          EXPORTS: getExportsIdentifier(asset, path.scope),
